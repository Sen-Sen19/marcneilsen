    ...
    if (nx >= 0 && nx < cols && ny >= 0 && ny < rows) {
      if (isErasing) {
        ...
        if (selectedMaterial === "Steam" || selectedMaterial === "Gas") {
          ...
          const px = nx * gridSize + gridSize / 2;
          const py = ny * gridSize + gridSize / 2;
          gasParticles.push(new GasParticle(px, py, selectedMaterial));
        } else {
          grid[ny][nx] = selectedMaterial;
        }
      }
...
class GasParticle {
  constructor(x, y, type) {
    const mat = gasMaterials[type];
    this.type = type;
    this.x = x;
    this.y = y;
    this.vx = (Math.random() - 0.5) * (mat.speed || 0.5);
    this.vy = -Math.random() * (mat.speed || 1);
    this.life = mat.life || 100;
    this.color = mat.color();
    this.transform = mat.transform || null;
  }

  update() {
    this.x += this.vx;
    this.y += this.vy * 0.5; // gentle upward drift
    this.life--;
  }
  ...
  draw(ctx) {
    ctx.beginPath();
    ctx.fillStyle = this.color;
    ctx.globalAlpha = Math.max(this.life / 100, 0.1);
...
function updateGasParticles() {
  for (let i = gasParticles.length - 1; i >= 0; i--) {
    ...
    // 🔒 Skip anything invalid or without proper methods
    if (!p || typeof p.update !== "function" || typeof p.isAlive !== "function") {
      console.warn("⚠️ Invalid entry in gasParticles:", p);
